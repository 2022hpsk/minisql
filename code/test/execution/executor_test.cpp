...
//  //第二条记录！
//  // Create values plan node
//  const1 = MakeConstantValueExpression(Field(kTypeInt, 1002));
//  const2 = MakeConstantValueExpression(Field(kTypeChar, const_cast<char *>("bbb"), 3, false));
//  const3 = MakeConstantValueExpression(Field(kTypeFloat, static_cast<float>(1.23)));
//  std::vector<std::vector<AbstractExpressionRef>> raw_values2{{const1, const2, const3}};
//  value_plan = std::make_shared<ValuesPlanNode>(nullptr, raw_values2);
//
//  // Create insert plan node
//  //TableInfo *table_info;
//  GetExecutorContext()->GetCatalog()->GetTable("table-1", table_info);
//  insert_plan = std::make_shared<InsertPlanNode>(nullptr, value_plan, "table-1");
//
//
//  // Execute insert plan, then iterate through table make sure that values were inserted
//  result_set.clear();
...
//  result_set.clear();
//
//  // SELECT * FROM table-1 where id = 1002;
//  const Schema *schema2 = table_info->GetSchema();
//  col_a = MakeColumnValueExpression(*schema2, 0, "id");
//  const500 = MakeConstantValueExpression(Field(kTypeInt, 1002));
//  predicate = MakeComparisonExpression(col_a, const500, "=");
//  scan_plan = make_shared<SeqScanPlanNode>(schema2, table_info->GetTableName(), predicate);
//
//  GetExecutionEngine()->ExecutePlan(scan_plan, &result_set, GetTxn(), GetExecutorContext());
//
//  // Size
...
//
//  // Value
//  ASSERT_TRUE(result_set[0].GetField(0)->CompareEquals(Field(kTypeInt, 1002)));
//  ASSERT_TRUE(result_set[0].GetField(1)->CompareEquals(Field(kTypeChar, const_cast<char *>("bbb"), 3, false)));
//  ASSERT_TRUE(result_set[0].GetField(2)->CompareEquals(Field(kTypeFloat, static_cast<float>(1.23))));
