...
TEST(DiskManagerTest, BitMapPageTest)
{
  ...
  for (uint32_t i = 0; i < num_pages; i++)
  {
    // if (bitmap->AllocatePage(ofs))
    // {
    // if (!bitmap->IsPageFree(i))
    // {
    //   ASSERT_EQ(i, ((unsigned char)128 >> 1));
    // }
    // }
  ...
  // ASSERT_EQ(233, bitmap->bytes[503]);
  // bitmap->DeAllocatePage(4030);
  for (auto v : page_set)
  {
    if (!bitmap->DeAllocatePage(v))
    {
      ASSERT_EQ(v, bitmap->bytes[503]);
    }
...
TEST(DiskManagerTest, FreePageAllocationTest)
{
  ...
  int extent_nums = 2;
  for (uint32_t i = 0; i < DiskManager::BITMAP_SIZE * extent_nums - 20; i++)
  {
  ...
  for (uint32_t i = DiskManager::BITMAP_SIZE * extent_nums - 20; i < DiskManager::BITMAP_SIZE * extent_nums; i++)
  {
    page_id_t page_id = disk_mgr->AllocatePage();
    DiskFileMetaPage *meta_page = reinterpret_cast<DiskFileMetaPage *>(disk_mgr->GetMetaData());
    EXPECT_EQ(i, page_id);
