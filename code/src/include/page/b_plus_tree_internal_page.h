...
#define INTERNAL_PAIR (sizeof(std::pair<GenericKey *, page_id_t>))
// 调整了一下原本的最大值定义
#define INTERNAL_PAGE_SIZE ((PAGE_SIZE - INTERNAL_PAGE_HEADER_SIZE) / INTERNAL_PAIR - 1)
/**
...
public:
  ...
  void Init(page_id_t page_id, page_id_t parent_id = INVALID_PAGE_ID, int key_size = UNDEFINED_SIZE,
            int max_size = INTERNAL_PAGE_SIZE); // 这里的max_size赋初值不要为0
