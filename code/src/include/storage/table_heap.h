...
private:
  ...
  explicit TableHeap(BufferPoolManager *buffer_pool_manager, Schema *schema, Transaction *txn,
                     LogManager *log_manager, LockManager *lock_manager) : buffer_pool_manager_(buffer_pool_manager),
                                                                           schema_(schema),
                                                                           ...
                                                                           lock_manager_(lock_manager)
  {
    ...
    TablePage *firstpage = reinterpret_cast<TablePage *>(this->buffer_pool_manager_->NewPage(this->first_page_id_));
    ASSERT(this->first_page_id_ >= 0, "New tableheap first page invalid");
    firstpage->Init(this->first_page_id_, INVALID_PAGE_ID, log_manager, txn);
    this->buffer_pool_manager_->UnpinPage(this->first_page_id_, true);
  };
